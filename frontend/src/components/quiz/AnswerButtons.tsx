    ...
    <div className="space-y-3">
        ...
        {question.options.map((option, i) => {
          ...
          return (
            <button
              ...
              disabled={isAnswered}
              className={cn(
                'flex items-start gap-3 p-4 rounded-lg border text-left transition-all duration-200 font-mono text-sm',
                !isAnswered && !isSelected && 'border-cyber-border bg-cyber-surface hover:border-neon-purple/50 hover:bg-neon-purple/5 hover:scale-[1.02] active:scale-[0.98]',
                !isAnswered && isSelected && 'border-neon-blue/50 bg-neon-blue/10 glow-border-blue scale-[1.02]',
            ...
            >
              <span
                className={cn(
                  'flex-shrink-0 w-7 h-7 rounded flex items-center justify-center text-xs font-bold border',
      ...
      {!isAnswered && selectedAnswer && (
        <button
          ...
          className="w-full py-3 rounded-lg border border-neon-purple/50 bg-neon-purple/10 text-neon-purple font-bold uppercase tracking-wider text-sm
